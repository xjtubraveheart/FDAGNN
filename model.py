...
class DiffGCN(nn.Module):
    def __init__(self,
        ...
        self.diff_layer.append(DiffAttention(g, in_dim, hidden_dim, feat_drop))
        for i in range(1, layer_num):
            self.diff_layer.append(DiffAttention(g, hidden_dim, hidden_dim, feat_drop))
        self.out_layer = nn.Linear(hidden_dim, out_dim, bias=False)
        nn.init.xavier_uniform_(self.out_layer.weight, gain=1.414)
        
    def forward(self, feat):
        h_e = []
        h = self.diff_layer[0](feat)
        h_e.append(h)
        for i in range(1, self.layer_num):
            h = self.diff_layer[i](h)
            h_e.append(h)
